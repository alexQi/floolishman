...
func StringArrayToIntArray(strs []string) []int {
...
func GenerateNumberSetFormString(setString string, splitString string) ([]int, error) {
	var numbers []int
	// 解析字符串，提取范围的起始值和结束值
	parts := strings.Split(setString, splitString)
	if len(parts) != 2 {
		return numbers, errors.New("Invalid range format")
	}
	start, err := strconv.Atoi(parts[0])
	if err != nil {
		return numbers, errors.New("Invalid start value")
	}
	end, err := strconv.Atoi(parts[1])
	if err != nil {
		return numbers, errors.New("Invalid end value")
	}
	// 生成数组
	for i := start; i <= end; i++ {
...
func IsInStringArray(array []string, target string) bool {
...
func IsInArray(array []int, target int) bool {
	for _, num := range array {
		if num == target {
			return true
		}
				...
				if iv.FieldByName(inFieldName).IsValid() && ov.FieldByName(outFieldName).CanSet() {
					if outFieldType == inFieldType {
						...
						if inFieldType.ConvertibleTo(outFieldType) {
							...
							utils.Log.Errorf("set attr failed: name [%s],type [%s] - [%s] ", outFieldName, inFieldType.String(), outFieldType.String())
						}
					}
				}
				break
		...
		if i.FieldByName(fieldName).IsValid() && o.FieldByName(fieldName).CanSet() {
			...
			if dataType == fieldType {
				...
				if dataType.ConvertibleTo(fieldType) {
					...
					utils.Log.Errorf("set attr failed: name [%s],type [%s] - [%s] ", fieldName, fieldType.String(), dataType.String())
				}
			}
		}
	}
