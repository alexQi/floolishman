...
func (s *StrategyService) openPosition(option model.PairOption, broker reference.Broker) {
	...
	if currentScore == 0 {
		utils.Log.Infof("[WAIT] Tendency: %s | Pair: %s | Price: %v | Strategy Count: %d, %d Matchers ", finalTendency, option.Pair, currentPirce, len(matchers), matcherCount)
		return
	}
	utils.Log.Infof(
	...
	for orderFlag, existOrders := range existOrderMap {
		...
		for _, positionOrder := range positionOrders {
			...
			if isProfitable == false {
				utils.Log.Infof(
					"[LOSS] Pair: %s | Price Order: %v, Current: %v | Quantity: %v",
					option.Pair,
					positionOrder.Price,
					currentPirce,
			...
			utils.Log.Infof(
				"[GROW] Pair: %s | Side: %s | Order Price: %v, Current: %v | Quantity: %v | Profit Ratio: %s | Loss Price: %v, Ratio:%s",
				option.Pair,
				positionOrder.Side,
				positionOrder.Price,
				currentPirce,
